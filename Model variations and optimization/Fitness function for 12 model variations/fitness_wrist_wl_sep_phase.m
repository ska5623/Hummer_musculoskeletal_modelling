function y = fitness_wrist_wl_sep_phase(paramu)
load insertion_and_axisdata.mat  %coordinate data for pect and supra insertion
load('torque_data_wing_corrected_final.mat'); %CFD obtained and shifted wing actuation torque
load skeletal_axis_data_wrist.mat  %HUP kinematic data
load wristlength.mat %one measure of secondary muscle length
load global_work_wrist.mat %Work data

NT = 100;
PB = [4.67 3.7 8.4571];  %raw coordinate data from CAD files
SB = [0 0.85 0];

%parameters
k1p = paramu(1,1)*0.001; %spring coefficients
k1s = paramu(1,2)*0.001; 
k2p = paramu(1,3)*0.001;
k2s = paramu(1,4)*0.001;
s1 = paramu(1,5); %PECT CPG amplitude
sf = paramu(1,6); %SUP CPG amplitude
s2 = sf*s1;
if s2 > 10
    s2 = 10;
end
phi = paramu(1,7); %phase of PMG
startangle = paramu(1,8); %in degree, for SUPRA
endangle = paramu(1,9); %in degree, for SUPRA
startpoint = paramu(1,10); 
endpoint = paramu(1,11);
startanglep1 = paramu(1,12); %in degree, for PECT
endanglep1 = paramu(1,13); %in degree, for PECT
startanglep2 = paramu(1,14); %in degree, for PECT
endanglep2 = paramu(1,15); %in degree, for PECT

p1d = paramu(1,16); %spring coefficient for DMG
s1d = paramu(1,17); %DMG muscle 1 amplitude
s2d = paramu(1,18); %DMG muscle 2 amplitude
meand = paramu(1,19); %offset for DMG
pshiftd = paramu(1,20); %active period for DMG muscle 1
sshiftd = paramu(1,21); %active period for DMG muscle 2
p1p = paramu(1,22); %spring coefficient for PiMG
s1p = paramu(1,23); %PiMG muscle 1 amplitude
s2p = paramu(1,24); %PiMG muscle 2 amplitude
meanp = paramu(1,25); %offset for PiMG
pshiftp = paramu(1,26); %active period for PiMG muscle 1
sshiftp = paramu(1,27); %active period for PiMG muscle 2
ofpf = paramu(1,28); %PECT offset from minimum length
ofsf = paramu(1,29); %SUPRA offset from minimum length
PBx = paramu(1,30); %PB insertion point offset
PBy = paramu(1,31); %PB insertion point offset
SBx = paramu(1,32); %SB insertion point offset
SBz = paramu(1,33); %SB insertion point offset
phid = paramu(1,34); %phase for DMG
phip = paramu(1,35); %phase for PiMG
dpsign = paramu(1,36); %Binary parameter for DMG muscle 1
dnsign = paramu(1,37); %Binary parameter for DMG muscle 2
ppsign = paramu(1,38); %Binary parameter for PiMG muscle 1
pnsign = paramu(1,39); %Binary parameter for PiMG muscle 2

%% Modification of the fixed body insertion points
PB(1,1) = PB(1,1) + PBx;
PB(1,2) = PB(1,2) + PBy;
SB(1,1) = SB(1,1) + SBx;
SB(1,3) = SB(1,3) + SBz;

%% Pect and SUPRA angle correction 
...
%% active force profile definition
%pect and supr
...
%% calculation of PECT direction correction and angle
...
%% PECT and SUPRA force and torque calculation, and DMG & PiMG torque calculation
...
for r = 1:1:NT 
         ...
         torquepitch(r,:) = ((p1p)*wlfinal(r,1) + meanp + Fcpgposp(1,r)/2500 + Fcpgnegp(1,r)/2500)*(-skllongfinal(:,r)');
...
%% work and power calculation for muscles 
...
%% cost estimation
y = 0;
y = y + 40*(0.0025450 - worktotal(100,1))^2;  %work part in cost

  for i = 1:1:100 %torque part in cost
